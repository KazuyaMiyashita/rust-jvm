...
trait ReaderWithCp {
    fn read(bytes: &[u8], offset: &mut usize, constant_pool: &Vec<constant_pool::CpInfo>) -> Result<Self> where Self: Sized;
}

trait VecReaderWithCp {
    fn read(bytes: &[u8], offset: &mut usize, constant_pool: &Vec<constant_pool::CpInfo>, num_of_items: usize) -> Result<Vec<Self>> where Self: Sized;
}
...
impl<T> VecReaderWithCp for T where T: ReaderWithCp {
    fn read(bytes: &[u8], offset: &mut usize, constant_pool: &Vec<constant_pool::CpInfo>, num_of_items: usize) -> Result<Vec<T>> where Self: Sized {
        let mut items: Vec<T> = Vec::new();
        for _ in 0..num_of_items {
...
impl Reader for ClassFile {
    fn read(bytes: &[u8], offset: &mut usize) -> Result<ClassFile> {
        ...
        let constant_pool: Vec<constant_pool::CpInfo> = VecReader::read(&bytes, &mut *offset, (constant_pool_count - 1) as usize)?;
